...
// Generic function with conditional type - unwrap non-promise values
export function unwrap<T>(value: T): T {
	return value
}

// Helper function to unwrap promises
export async function unwrapPromise<T>(value: Promise<T>): Promise<T> {
	return value
}

// Generic function that handles both cases
export function getValue<T>(value: T | Promise<T>): T extends Promise<infer U> ? Promise<U> : T {
	return value as any
}
