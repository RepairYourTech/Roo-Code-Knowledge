...
async function testComprehensiveQueryValidation() {
	try {
		...
		if (
			!parsers["ts"].query ||
			!parsers["ts"].query.captureNames ||
			parsers["ts"].query.captureNames.length === 0
		) {
			throw new Error("TypeScript parser has no capture names")
		}
		if (
			!parsers["js"].query ||
			!parsers["js"].query.captureNames ||
			parsers["js"].query.captureNames.length === 0
		) {
		...
		// Test modern code snippets with expected captures
		const tsSnippets = [
			{
